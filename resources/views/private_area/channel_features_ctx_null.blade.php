        ...
        <div class="col-6 bg-light p-4 rounded-sm">
            <form id="form_channel" method="POST" action="{{$action}}">
                ...
                <div class="row mt-1 small">
                    <div class="col-12">Areas:</div>
                </div>
                <div class="row form-group mt-1">
                    <div class="col-12">
                        <select id="coordinates" name="coordinates" readonly="true" size="6" style="width: 250px;">
                            @if ($hasCoords)
                                @foreach($area_names as $area_name)
                                    <option id="1">{{$area_name}}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
        ...
        @if (!is_null($camera_url))
          <p class="small">Press <strong>Left Click</strong> to draw a point. <strong>CTRL+Click</strong> or <strong>Right Click</strong> to close the polygon. You need at least three points.</p>
            ...
            <div class="mt-2 text-left">
                    ...
                    <button class="btn btn-secondary" onclick="clear_canvas()">Clear</button>
            </div>
        @endif
        </div>
    </div>
...
function print_area(coords=null){
    console.log("print_area:"+coords);
...
@if ($hasCoords)
    var list_coords ={!!json_encode($coords)!!};
    $( document ).ready(function() {
        console.log( "ready!" );
        new_canvas();
        list_coords.forEach(function(area_coords) {
